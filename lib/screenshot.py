"""
Presto screenshots, pushed over TCP. Shared device module (deploy to :lib/).

Writing the framebuffer to flash deadlocks on the current Presto firmware (see
badgeware-presto's FLASH_WRITES_HANG), so instead the app runs a tiny TCP
...
front buffer. The host does the RGB565 -> PNG conversion.
    ...
    app:   screenshot.serve_init(PORT)                    once, after WiFi is up
           screenshot.serve_poll(display, presto.buffer)  every frame

    host:  python3 <tool>_pull.py <presto-ip>
