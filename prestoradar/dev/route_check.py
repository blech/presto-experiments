...
Position lookup uses adsb.lol's network-wide /v2/hex and /v2/callsign
endpoints, not settings.py's local radius -- the aircraft doesn't need to be
anywhere near your radar's centre for this to work.
"""
...
from settings import USER_AGENT  # noqa: E402
...
# --- position lookup (network-wide, not settings.py's local radius) -------

async def _find_aircraft(path):
    ...
    if not ac:
        return None, "not currently visible on adsb.lol (not airborne, or out of coverage)"
...
async def find_by_hex(hex_id):
    return await _find_aircraft(f"/v2/hex/{hex_id}")


async def find_by_callsign(callsign):
    return await _find_aircraft(f"/v2/callsign/{callsign}")
...
async def run(args):
    if args.hex:
        plane, err = await find_by_hex(args.hex)
    else:
        plane, err = await find_by_callsign(args.callsign.strip())
    if err:
        print(err)
        return 1
